...
## Option 3: Silent Launcher (best for sharing with non-technical friends)

If a friend already has Python, Node.js, and Ollama installed and just wants to open
the app like a normal program — no terminal windows, no typing — use
**`start_silent.vbs`**. It also auto-updates from GitHub before starting, so they
always get your latest pushed changes without re-downloading anything.

**One-time setup (still required once):**

1. Get the code onto their computer with `git clone` (not a zip download — the
   auto-update step needs this to be a real git repository):
   ```
   git clone https://github.com/GAIIIZZZ/income-tracker.git
   ```
2. Double-click **`setup.bat`** once (same as Option 1) to install dependencies.
3. Make sure Ollama is running and `ollama pull qwen2.5:1.5b` has been run.

**Every time after that, to use the app:**

1. Double-click **`start_silent.vbs`**.
   - No windows pop up — it silently pulls the latest code from GitHub, starts the
     backend and frontend in the background, and opens `http://localhost:3000` in
     the browser automatically after a few seconds.
   - (Optional, recommended) Right-click `start_silent.vbs` → **Send to → Desktop
     (create shortcut)**, then rename the shortcut to something like "Income
     Tracker" — now it's just a normal desktop icon to double-click.
2. Use the app in the browser as usual.
3. When done, double-click **`stop_silent.bat`** to shut the backend and frontend
   down. (Since nothing is visible, there's no window to just close or Ctrl+C —
   this finds and stops whatever is running on ports 8000/3000.)

**If something doesn't work:** since there's no visible terminal to show errors,
`start_silent.vbs` writes everything it would have printed to log files instead —
check `update_log.txt` (project root), `backend_log.txt` (in `backend/`), and
`frontend_log.txt` (in `frontend/`) for the actual error message.

---

## Option 4: Cloud Alternative (Streamlit + Supabase + Groq) — a shareable link, no local setup for friends

There's also a separate, optional cloud version in the **`cloud/`** folder — same idea (OCR
bank slips → AI-structure → review → track income) but deployable for free as a public web
link, instead of only running on `localhost`. It swaps the local SQLite database for Supabase
and the local Ollama LLM for Groq's free hosted API (a free web host can't run Ollama itself),
so it's no longer 100% private the way the local app is — the trade-off for not needing anyone
to install anything.

Full setup and free-deploy steps are in **[`cloud/README.md`](cloud/README.md)**. It's a real,
independent build — it doesn't replace or require the local app.
...
start_silent.vbs        hidden, auto-updating launcher, no terminal windows (Option 3)
stop_silent.bat         stops what start_silent.vbs started
...
cloud/                  optional cloud alt: Streamlit + Supabase + Groq (Option 4, see cloud/README.md)
```
...
## Option 3: ตัวเปิดแบบเงียบ (Silent Launcher) — เหมาะสำหรับแชร์ให้เพื่อนที่ไม่ถนัดสายเทคนิค

ถ้าเพื่อนของคุณติดตั้ง Python, Node.js และ Ollama ไว้แล้ว และแค่อยากเปิดแอปเหมือนโปรแกรมทั่วไป
— ไม่ต้องมีหน้าต่างเทอร์มินัล ไม่ต้องพิมพ์อะไรเลย — ให้ใช้ **`start_silent.vbs`** แทน
ตัวนี้จะอัปเดตโค้ดล่าสุดจาก GitHub ให้อัตโนมัติก่อนเริ่มทำงานด้วย เพื่อนจะได้โค้ดล่าสุดที่คุณ push
ไว้เสมอโดยไม่ต้องดาวน์โหลดใหม่เอง

**ขั้นตอนตั้งค่าครั้งแรก (ยังต้องทำครั้งเดียวก่อน):**

1. โคลนโค้ดลงเครื่องด้วย `git clone` (ห้ามดาวน์โหลดเป็น zip — ขั้นตอนอัปเดตอัตโนมัติต้องการให้
   เป็น git repository จริง ๆ):
   ```
   git clone https://github.com/GAIIIZZZ/income-tracker.git
   ```
2. ดับเบิลคลิก **`setup.bat`** หนึ่งครั้ง (เหมือน Option 1) เพื่อติดตั้งไลบรารีที่จำเป็น
3. ตรวจสอบว่า Ollama กำลังทำงานอยู่ และรัน `ollama pull qwen2.5:1.5b` แล้ว

**ทุกครั้งหลังจากนั้น เวลาจะใช้งานแอป:**

1. ดับเบิลคลิก **`start_silent.vbs`**
   - จะไม่มีหน้าต่างใด ๆ เด้งขึ้นมาเลย — มันจะดึงโค้ดล่าสุดจาก GitHub มาอัปเดตให้อัตโนมัติ
     เริ่มรัน backend และ frontend เบื้องหลัง แล้วเปิด `http://localhost:3000` ในเบราว์เซอร์ให้เอง
     หลังจากรอสักครู่
   - (ทางเลือกเสริม แนะนำ) คลิกขวาที่ `start_silent.vbs` → **Send to → Desktop (create
     shortcut)** แล้วเปลี่ยนชื่อ shortcut เป็น "Income Tracker" — ต่อไปก็แค่ดับเบิลคลิกไอคอนบน
     เดสก์ท็อปเหมือนโปรแกรมทั่วไป
2. ใช้งานแอปในเบราว์เซอร์ตามปกติ
3. เมื่อใช้เสร็จแล้ว ดับเบิลคลิก **`stop_silent.bat`** เพื่อปิด backend และ frontend
   (เนื่องจากไม่มีหน้าต่างให้เห็น จึงไม่มีอะไรให้ปิดหรือกด Ctrl+C — ไฟล์นี้จะค้นหาและหยุด
   โปรเซสที่ทำงานอยู่ที่พอร์ต 8000/3000 แทน)

**ถ้ามีอะไรไม่ทำงาน:** เนื่องจากไม่มีเทอร์มินัลให้เห็น error `start_silent.vbs` จะเขียนทุกอย่างที่
ควรจะขึ้นบนหน้าจอลงไฟล์ log แทน — เช็คที่ `update_log.txt` (โฟลเดอร์หลักของโปรเจกต์),
`backend_log.txt` (ในโฟลเดอร์ `backend/`) และ `frontend_log.txt` (ในโฟลเดอร์ `frontend/`)
เพื่อดู error ที่แท้จริง

---

## Option 4: ทางเลือกแบบ Cloud (Streamlit + Supabase + Groq) — เป็นลิงก์แชร์ได้ ไม่ต้องติดตั้งอะไรฝั่งเพื่อน

มีเวอร์ชัน cloud แยกต่างหากอยู่ในโฟลเดอร์ **`cloud/`** ด้วย — แนวคิดเดียวกัน (OCR สลิปธนาคาร →
จัดโครงสร้างด้วย AI → ตรวจสอบ → ติดตามรายรับ) แต่ deploy ขึ้นเป็นลิงก์เว็บสาธารณะได้ฟรี แทนที่จะรัน
ได้แค่บน `localhost` โดยเปลี่ยนจากฐานข้อมูล SQLite ในเครื่องเป็น Supabase และเปลี่ยนจาก Ollama
(local) เป็น Groq API ที่ให้ใช้ฟรี (เพราะ host ฟรีไม่มีทางรัน Ollama เองได้) ซึ่งหมายความว่าจะไม่
private 100% เหมือนแอปเวอร์ชัน local อีกต่อไป — เป็นสิ่งที่แลกมากับการที่ไม่ต้องให้ใครติดตั้งอะไรเลย

ขั้นตอนตั้งค่าและ deploy แบบฟรีทั้งหมดอยู่ที่ **[`cloud/README.md`](cloud/README.md)** เป็นเวอร์ชัน
ที่แยกออกมาต่างหากจริง ๆ — ไม่ได้แทนที่หรือจำเป็นต้องพึ่งพาแอปเวอร์ชัน local แต่อย่างใด
...
start_silent.vbs        ตัวเปิดแบบเงียบ อัปเดตอัตโนมัติ ไม่มีหน้าต่างเทอร์มินัล (Option 3)
stop_silent.bat         ใช้ปิดสิ่งที่ start_silent.vbs เปิดไว้
...
cloud/                  ทางเลือก Cloud: Streamlit + Supabase + Groq (Option 4 ดูที่ cloud/README.md)
```
