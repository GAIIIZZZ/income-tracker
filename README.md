...
| เบราว์เซอร์ไม่ขึ้นอะไรเลย / connection refused ที่ `localhost:3000` | เซิร์ฟเวอร์ frontend ไม่ได้ทำงาน หรือ backend ไม่ได้ทำงาน (frontend ต้องพึ่งพา backend ที่ทำงานอยู่ก่อน) ตรวจสอบหน้าต่างเทอร์มินัลทั้งสองว่ามี error หรือไม่ |
| พอร์ต (`8000` หรือ `3000`) ถูกใช้งานอยู่แล้ว | มีแอปนี้อีกชุดหนึ่ง (หรือโปรแกรมอื่น) กำลังใช้พอร์ตนั้นอยู่ ให้ปิดแอปที่รันอยู่ก่อนหน้านี้ก่อน |
